...
# i=1;ped_val=ped_test; beta.info=beta.list1[beta.list1$CHR==i,1:3];beta.all=beta.list1[beta.list1$CHR==i,4:k]; BedFileReader_new=BedFileReaders[[i]]
...
Calculate_PRS_direct <- function(ped_val, beta.info, beta.all, BedFileReader_new, plink_file){
    ...
    cat('length of flag: ',length(flag),'\n')
...
PRStr_tuning<-function(Beta.all, ped_val_file, tempfile, Covar_name,Y_name, Ytype, lr_list, iter, plink_file){
    tic('beta list prep')
    ...
    ## Uncomment when doing validation only
    # sourceCpp("/media/leelabsg-storage1/bokeum/TLPRS/source/BedFileReader.cpp")
    # setClass("BedFileReader", representation( pointer = "externalptr" ) )
    # BedFileReader_method <- function(name) {paste( "BedFileReader", name, sep = "__" ) }
    # setMethod( "$", "BedFileReader", function(x, name ) {function(...) .Call(BedFileReader_method(name),x@pointer, ... )})
    ...
    # assign bedfilereader pointers to a list
    BedFileReaders <- BedFileReader_prep(plink_file)
    ...
    tic('calculating PRS')
        ...
        for (i in unique(beta.info$CHR)) {
            ...
            partial_PRS <- Calculate_PRS_direct(
                ...
                beta.all[beta.info$CHR==i,], #col: c(1:101, 302:401, 602:701, 902:1001, 1202:1301, 1502:1601)
        ...
        if (!file.exists(target_sumstats_train_file)){out1="The target summary statistic train file does not exist!"} else {
    		...
    		if (ncol(temp)==4){   
                if (sum(colnames(temp) %in% c("SNP", "A1", "cor", "N"))==4){} else{
              	...
              	if (ncol(temp)>4){
                    if (sum(colnames(temp) %in% c("SNP", "A1", "cor", "N"))<4){ 
                        out1="The structure of target_sumstats_train_file is wrong!"
                    }
                } else {
                    out1="The structure of target_sumstats_train_file is wrong!"
...
PRS_tuning_pv_byLR <- function(beta.byL, betaRho.byL, betaG.byL, lr_list){
  ...
  out.final$best.beta = as.data.frame(beta.byL)[,c(1:3,9,9+flag)] #SNP, CHR, A1, Beta2, best.beta
  out.final$R2.list = R2.byL
#   out.final$beta.list <- beta.byL

  return(out.final)
}

Calculate_R <- function(beta_train, test_summ, geno_ref){ #geno_ref
    n_test = test_summ$N[1]
    
    # test_summ = test_summ[test_summ$SNP %in% geno_info$SNP,]

    # scaling genotype 
    # geno_ref_ <- scale(geno_ref) #sweep(geno_ref, 2, colMeans(geno_ref),'-')

    R_num = sum(beta_train*test_summ$cor)^2 #* n_test
    R_denom = sum((geno_ref*beta_train)^2)
    # PRS = geno_ref%*%beta_train

    return(list(R_num,R_denom))
}